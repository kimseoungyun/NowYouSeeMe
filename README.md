...
JACK 파이프라인에서는 동일한 비디오를
Metadata 추출, Core AD 생성, STT 분석 단계에서
중복 업로드하지 않고,
Gemini Files API를 통해 **한 번만 업로드한 파일을 모든 Phase에서 재사용**한다.

이를 통해 불필요한 입력 토큰 사용을 방지하고,
멀티 프롬프트 기반 구조에서도
전체 토큰 비용을 안정적으로 관리할 수 있도록 설계하였다.
...
Phase 2의 모든 입력 생성 단계는
`asyncio.gather` 기반 비동기 처리로 구성되어,
Metadata 추출, Core AD 생성, STT 분석을 **병렬로 동시에 실행**한다.

이를 통해 네트워크 I/O 지연으로 인한
전체 처리 시간 증가를 방지하고,
영상 길이 증가에도 처리 시간이 선형적으로 증가하지 않도록 설계하였다.
...
```
#### 입력 비디오 형태에 따른 토큰 영향 분석

JACK 모델 설계 과정에서,
음성 제거, 해상도 축소, 프레임 품질 저하 등
다양한 형태의 비디오를 입력으로 사용하여
토큰 사용량 변화를 실험하였다.

그 결과, 비디오의 시각적 품질이나
오디오 포함 여부와 무관하게,
**Gemini Files API를 통해 업로드된 비디오의 입력 토큰 값은
항상 동일하게 유지됨**을 확인하였다.

이를 통해 토큰 비용에 가장 큰 영향을 미치는 요소는
입력 비디오 자체가 아닌,
프롬프트 구조와 후처리 단계에서 생성되는 텍스트 길이임을 명확히 하였으며,
이후 최적화 전략은 프롬프트 분리 및 압축 설계에 집중하였다.
...
JACK 모델 설계 과정에서 단일 temperature 값에 의존할 경우,
AD의 길이, 표현 밀도, 중요 시각 정보 선택에서
미세한 변동이 발생하는 문제가 확인되었다.

초기 설계 단계에서는 이러한 변동성을 완화하기 위해
여러 temperature 값을 병렬 또는 순차적으로 적용하여
출력을 보정하는 방식을 고안하였다.
그러나 해당 방식은 추가적인 API 호출로 인한
토큰 사용량 증가에 비해,
실질적인 보정 효과가 제한적임을 확인하였다.

이에 따라 AD 생성 단계에서는
출력 일관성과 재현성을 우선시하여
**저온 설정(temperature = 0)**을 단일 기준으로 채택하였다.

해당 설정은 재실행 시 결과 변동을 최소화하면서도,
불필요한 토큰 소모를 방지하고,
Stage 2 압축 및 후처리 단계에서
문자 수 제어와 문장 안정성을 확보하는 데
가장 효율적인 선택임을 실험적으로 확인하였다.
...
- **응답 누락 대비 재호출 로직**
  - Gemini API 호출 후 응답이 비어 있거나
    JSON 파싱이 불가능한 경우를 실패 상태로 판단
  - 동일 요청을 최대 3회까지 재호출하여
    일시적 네트워크 오류 또는 모델 응답 실패에 대비
  - 이를 통해 전체 파이프라인 중단 없이
    AD 생성의 안정성을 확보하였다.
