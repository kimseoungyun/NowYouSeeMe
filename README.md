...
- **중요한 Description의 정의**
  - 모든 시각 정보를 나열하지 않고,
    장면의 이해에 결정적인 영향을 미치는 요소를 우선적으로 설명 대상으로 선정한다.
  - 주요 기준은 다음과 같다.
    - 장면(Scene)의 전환 여부
    - 주요 인물의 등장 또는 행동 변화
    - 서사 흐름상 중요한 사건 발생
- **시간 제약 인식(Time-aware Generation)**
  - AD는 항상 특정 무음 구간 또는 제한된 재생 구간 내에서 제공되어야 하므로,
    생성 단계에서부터 시간 제약을 핵심 조건으로 고려한다.
- **최소 생성 길이 기준**
  - 지나치게 짧은 구간에서는 의미 있는 설명 전달이 어렵다고 판단하여,
    AD 생성의 최소 길이를 약 **2.5~3초 이상**으로 설정하였다.
  - 해당 기준 미만의 구간에 대해서는
    설명 생략 또는 구간 병합과 같은 대안 전략을 적용한다.
